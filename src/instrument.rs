...
impl<'tree> Instrumenter {
    ...
    ///    )
    /// ```
    fn handle_call_to_field_expression(&mut self, call: &Node, code: &str) {
        ...
        match fn_name_str.is_construct() {
            /* auto *NI = OI->clone(); */
        ...
        if fn_name_str.is_replacement() {
            ...
            let field_operator = function.child(1).unwrap().to_source(code);
    ...
    fn handle_call_to_qualified_identifier(&mut self, call: &Node, code: &str) {
        assert_eq!(call.kind(), "call_expression");
        
        let function = call.child_by_field_name("function").unwrap();
        let fn_name_str = function.to_source(code);
        if let Some(ConstructKind::Creating) = fn_name_str.is_construct() {
    ...
    fn handle_call_to_identifier(&mut self, call: &Node, code: &str) {
        assert_eq!(call.kind(), "call_expression");
    }

    fn collect_fn_body_edit(&mut self, fn_def: &Node, code: &str) {
        assert_eq!(fn_def.kind(), "function_definition");
        if fn_def.child_by_field_name("body").is_none() {
            return ;
        }
        ...
        /* New expressions */
        let news = get_children_of_kind(&body, "new_expression");
        for new in news {
    ...
    fn collect_edits(&mut self, code: &str) {
        ...
        for fn_def in get_children_of_kind(&root_node, "function_definition") {
            self.collect_fn_edit(&fn_def, code);
        }
